...
DFU_ERRCODE bl_fota_lzma_decompress(U32 src_addr, U32 srcLen)
{
    ...
    int i;
    SRes res = SZ_OK;
    hal_aes_status_t err = HAL_AES_STATUS_OK;
    ...
    {
        // inSize: ¥Ø«eÅª¶i inBuffer ªº¸ê®Æ size
        // inPos: ¥Ø«e inBuffer ¤¤ªº¸ê®Æ³B²z¨ì­þ
        // inPos: used size in inBuffer.
        ...
        for (;;)
        {
            ...
            if (inPos == inSize)
            {
                ...
                inPos = 0; // ªí¥Ü inBuffer ¤¤ªº¸ê®Æ³£¬O·sªº¡A»Ý­n³B²z
                srcIdx += inSize;
            }

            // inProcessed: un-decompressed data size in each segment of source data.
            SizeT inProcessed = inSize - inPos; // inBuffer ¤¤©|¥¼³B²zªº¸ê®Æ¤j¤p

            // outProcessed: unused space size in outBuf.
            SizeT outProcessed = OUT_BUF_SIZE - outPos; // outBuffer ÁÙ¦³¦h¤ÖªÅ¶¡¥i¥Î
...
DFU_ERRCODE bl_fota_move_data(VOID)
{
    ...
    {
        bl_gen_each_4k_target_addr_table();
    ...
    {
        err_code = bl_fota_flash_erase((U32)g_movers[i].destination_address, g_movers[i].length);
        if (err_code != DFU_ERR_SUCCESS)
        {
            return err_code;
        }

        err_code = bl_fota_flash_verify_erase((U32)g_movers[i].destination_address, g_movers[i].length);
        if (err_code != DFU_ERR_SUCCESS)
        {
            return err_code;
        }
...
void bl_fota_process (void)
{
    ...
    if (err_code != DFU_ERR_SUCCESS)
    {
        ...
        if(err_code == DFU_ERR_AES_IV_NOT_MATCH || err_code == DFU_ERR_AES_DECRYPT_FAIL)
        {
            result = FOTA_STATE_LOADER_PACKAGE_NG;
        }
        else
